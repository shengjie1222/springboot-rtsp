...
public class Stream {
	...
	public Stream(StreamName streamName) {
		...
		if (Options.liveConfig.isSaveFlvFile()) {
			createFileStream();
		}
	}
...
//		buf.writeInt(0); // timestamp + timestampExtended
	...
	private void createFileStream() {
		String fName = Options.liveConfig.getSaveFlVFilePath() + File.separator +
				streamName.getApp() +
				"_" +
				streamName.getName() +
				"_" +
		...
		try {
			File parentFile = new File(Options.liveConfig.getSaveFlVFilePath());
			parentFile.mkdirs();
			f.createNewFile();
	...
	public synchronized void sendEofToAllSubscriberAndClose() {
		if (Options.liveConfig.isSaveFlvFile() && flvout != null) {
			try {
				flvout.close();
			} catch (IOException e) {
				log.error("close file:{} failed", flvout,e);
			}
		}
